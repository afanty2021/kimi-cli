...
- **差异显示**: WriteFile 和 StrReplaceFile 包含差异显示
- **工具查找**: `KimiToolset.find` 方法按类或名称查找工具
- **显示构建**: `ToolResultBuilder.display` 方法添加显示块

### 会话管理改进

- **命名会话**: 支持通过 ID 创建和恢复会话
- **会话列表**: `/sessions` 命令列出和切换会话
- **自动清理**: 删除空会话，忽略空上下文会话

### 斜杠命令系统

- **两级命令**: Shell 级和 KimiSoul 级命令
- **命令广播**: 向 ACP 客户端广播可用命令
- **新命令**:
  - `/sessions` - 会话管理
  - `/mcp` - MCP 状态显示
  - `/usage` - API 使用情况
...
- **Vertex AI**: 新增 Google Vertex AI 提供商支持
- **Gemini 更新**: Gemini Developer API 提供商名称从 `google_genai` 改为 `gemini`
